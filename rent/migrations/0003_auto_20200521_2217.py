# Generated by Django 3.0.3 on 2020-05-21 14:17

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('rent', '0002_auto_20200521_1139'),
    ]

    operations = [
        migrations.AddField(
            model_name='rentorder',
            name='begintime',
            field=models.DateField(auto_now_add=True, default=None, verbose_name='开始时间'),
            preserve_default=False,
        ),
        migrations.AddField(
            model_name='rentorder',
            name='judge',
            field=models.BooleanField(default=False, verbose_name='审核结果'),
        ),
        migrations.AddField(
            model_name='rentorder',
            name='overtime',
            field=models.DateField(default=None, verbose_name='结束时间'),
        ),
        migrations.AddField(
            model_name='rentorder',
            name='type',
            field=models.CharField(choices=[('short', '短期'), ('long', '长期')], default=None, max_length=32, verbose_name='类型'),
            preserve_default=False,
        ),
    ]
